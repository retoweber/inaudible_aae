...
def compute_mfcc_numpy(audio, **kwargs):
    """
    Compute the MFCC for a given audio waveform. This is
    identical to how DeepSpeech does it, but does it all in
    TensorFlow so that we can differentiate through it.
    """
    batch_size, size = audio.shape
    audio = audio.astype('float32')


    # 1. Pre-emphasizer, a high-pass filter
    audio = np.concatenate((audio[:, :1], audio[:, 1:] - 0.97*audio[:, :-1], np.zeros((batch_size,1000),dtype=np.float32)), 1)

    # 2. windowing into frames of 320 samples, overlapping
    windowed = np.stack([audio[:, i:i+400] for i in range(0,size-320,160)],1)

    # 3. Take the FFT to convert to frequency space
    ffted = np.fft.rfft(windowed[0], 512)
    ffted = 1.0 / 512 * np.square(np.abs(ffted))
    ffted = np.array([ffted])

    # 4. Compute the Mel windowing of the FFT
    energy = np.sum(ffted,axis=2)+1e-30
    filters = np.load("filterbanks.npy").T
    feat = np.matmul(ffted, np.array([filters]*batch_size,dtype=np.float32))+1e-30

    # 5. Take the DCT again, because why not
    feat = np.log(feat)
    feat = fftpack.dct(feat, type=2, norm='ortho')[:,:,:26]

    # 6. Amplify high frequencies for some reason
    _,nframes,ncoeff = feat.shape
    n = np.arange(ncoeff)
    ...
    feat = np.concatenate((np.reshape(np.log(energy),(-1,width,1)), feat[:, :, 1:]), axis=2)

    return feat
 

def compute_mfcc_tensorflow(audio, **kwargs):
    """
    Compute the MFCC for a given audio waveform. This is
    identical to how DeepSpeech does it, but does it all in
    TensorFlow so that we can differentiate through it.
...
def get_logits(new_input, length, first=[]):
    ...
    new_input_to_mfcc = compute_mfcc_tensorflow(new_input)[:, ::2]
